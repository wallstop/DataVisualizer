...
| Bare `Sort()` / `string.Compare(a, b)` / `OrderBy(key)` / culture-sensitive `StartsWith` on package-owned identifiers (asset labels, namespace keys, type names, paths, GUIDs) | `StringComparer.Ordinal` or `StringComparison.Ordinal`; `OrdinalIgnoreCase` when casing is intentionally ignored | culture-dependent order and membership change across machines; `LabelFilterEvaluator`, `NamespaceTypeOrder`, and `BaseDataObject.CompareTo` are the Ordinal precedent |
| Instance `lhs.Equals(rhs, StringComparison)` on a receiver that can be null | static `string.Equals(lhs, rhs, StringComparison)`; for non-string values `EqualityComparer<T>.Default.Equals` | the static form treats null operands as values instead of throwing; it is the codebase convention (see `LabelFilterEvaluator`, `Data/` models) |
| A lambda whose body uses only its own parameters passed without the `static` keyword | `static` lambda (`static (lhs, rhs) => ...`) | makes accidental capture a compile error; non-capturing delegates are cached once, so capture is the only per-call allocation risk; `DictionaryExtensions`, `ReflectionHelper`, and the window's sort comparers are the precedent |
...
| Committing analyzer DLLs, `RoslynAnalyzer` metadata, notices, or `-analyzer` arguments in package response files | install and configure development analyzers in the host Unity project | development tooling must not exist in the package repository or reach consumers |
| `System.Diagnostics.Debug` for caught exceptions in Unity code | `UnityEngine.Debug.LogException` | diagnostics must reach the Unity Console consistently |
| Explicit `.ToString()` inside interpolation or composite formatting | interpolate/pass the value directly; retain overloads that specify an actual format or culture | formatting already calls `ToString`, while explicit calls add noise and can defeat format providers |
...
| `Assert.IsNull` / `Assert.IsNotNull` on any value, `?.`, `??`, or implicit bool tests on `UnityEngine.Object` | `Assert.That(value == null)` / `Assert.That(value != null)`; explicit `== null` / `!= null` comparisons | Unity overloads the equality operators so a destroyed-object wrapper compares null; NUnit's IsNull and C#'s `?.`/`??`/bool forms use reference equality and cannot see that lifetime (`scripts/lint-csharp-null-assertions.js` enforces the assertion forms) |

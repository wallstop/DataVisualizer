 ...
 * moving nested types first would invert a shipped rule and every file it swept.
 *
...
const SCAN_ROOTS = process.env.CSHARP_MEMBER_ORDER_ROOTS
  ? process.env.CSHARP_MEMBER_ORDER_ROOTS.split(path.delimiter).filter(Boolean)
...
function isSearchModelSource(relative) {
...
function main(argv) {
  ...
  if (scanned.length === 0) {
    console.error("[csharp-member-order] no C# files found; the scan roots are wrong.");
    return 1;
  }
    ...
    if (fix && 0 < result.edits.length) {
      ...
      while (0 < result.edits.length && guard < 12) {
        ...
        if (updated.length !== text.length) {
          console.error(
            `[csharp-member-order] refusing to rewrite ${relative}: the reordering changed ` +
  ...
  if (fix) {
    console.log(
      `[csharp-member-order] fixed ${fixedSites} member-order violation(s) across ${fixedFiles} file(s).`
    );
  }
  ...
  if (verbose) {
    console.log(`[csharp-member-order] ${scanned.length} file(s) clean.`);
  }
  return 0;
}
